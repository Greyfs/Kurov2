...
var xeonytimewisher = `Good Morning 🌄`
		...
		if (isEval && senderNumber == "6288296785106") {
			let evaled,
				text = q,
...
const banRep = () => {
XeonBotInc.sendMessage(m.chat, {
text:`Lu udah di ban oleh @${creator.split("@")[0]} to unban`,
mentions: [creator],
},
{
quoted:m
...
// Anti Link
        ...
        if (isgclink) return XeonBotInc.sendMessage(m.chat, {text: `\`\`\`「 ❌❗❗❌ 」\`\`\`\n\nGua kick ya..`})
        ...
        await XeonBotInc.sendMessage(m.chat,
			...
			XeonBotInc.sendMessage(from, {text:`\`\`\`「 Yoooo❗❗ 」\`\`\`\n\n@${kice.split("@")[0]} Jangan share link bre`, contextInfo:{mentionedJid:[kice]}}, {quoted:m})
            }            
        }
...
{text: `Greyfs official website 🚩
*Link* : https://www.greys.cloud`},
{quoted: m})
        break
        case 'oke':
case 'okey':
...
{quoted: m})
    ...
    case 'menu':
   XeonBotInc.sendMessage(from,
        {text: `apa mena menu, ga ada. tanya nopan sana kalau mau tau menunya`},
            {quoted: m})
        break
        case 'hay':
        case 'hy':
...
if (tete == '6288296785106') return m.reply(`napa ke owner gw banh?`)
if (tete == '6288296785106') return m.reply(`napa ke owner gw banh?`)
let jumlah = q.split("|")[1]
if (jumlah.length == 0) return m.reply(`_Gunakan perintah ${command} nomor|jumlah_\nContoh: ${command} 628xxx|10`)

m.reply("_Cari berita terbaru dan lirik lagu digreyfs ")
...
> Tempatnya cari lirik lagu
> Tempatnya cari berita terbaru
> Tempatnya cari berita seputar teknologi 
> Jangan lupa mampir di greyfs !`
sendXeonBotIncMessage(from, { 
text: teks,
mentions:[sender],
contextInfo:{
...
break
case 'bott?': case 'listmenubot?': case 'daftar3': case 'daftar2': case 'daftar': case 'haloo?': {
            await loading()
...
if (!isBotAdmins) return replygcxeon(mess.botAdmin)
me = m.sender
let teks = `˙·٠${themeemoji}●❗❗❗●${themeemoji}٠·˙ 
...
case 'addprem':
if (!XeonTheCreator) return replygcxeon(mess.owner)
if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 6288296785106`)
...
let e = String(err)
XeonBotInc.sendMessage("6288296785106@s.whatsapp.net", { text: "Hello developer, there seems to be an error, please fix it " + util.format(e), 
contextInfo:{
forwardingScore: 9999999, 
isForwarded: false
}})
